...
const CategoryTargetPage = () => {
  ...
  useEffect(() => {
    const fetchTargets = async () => {
      ...
      try {
        const res = await axios.get("https://gvi-pos-server.vercel.app/categoryTargets", {
          params: { year, month }
        });
    ...
    try {
      ...
      const updates = Object.entries(tempTargets)
        ...
        .map(([userID, targets]) => {
          ...
          return axios.post("https://gvi-pos-server.vercel.app/categoryTargets", {
            userID,
            year: parseInt(year),
            month: parseInt(month),
            targets: targetsArray
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex-1 overflow-y-auto p-6">
          <div className="max-w-full mx-auto bg-white rounded-lg shadow-md overflow-hidden">
            <div className="p-6">
              <h2 className="text-2xl font-semibold text-gray-800 mb-6">Category-wise Targets</h2>
                    ...
                    <thead className="bg-gray-50 sticky top-0">
                      <tr>
                        ...
                        {categories.map(category => (
                          <th key={category} scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                            {category}
                          </th>
                        ))}
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {users.map(user => (
                        <tr key={user._id} className="hover:bg-gray-50">
                          ...
                          {categories.map(category => (
                            <td key={`${user._id}-${category}`} className="px-2 py-1 whitespace-nowrap">
                              <input
                                type="number"
                                ...
                                onChange={(e) => handleTargetChange(user._id, category, e.target.value)}
                              />
                            </td>
                          ))}
