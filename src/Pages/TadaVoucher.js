...
const TadaVoucher = () => {
  ...
  const toggleCheckbox = (field) => {
    setDailyExpense(prev => ({
      ...
      hqExHqAmount: (field === 'hq' && !prev.exHq) || (field === 'exHq' && !prev.hq) ? '' : prev.hqExHqAmount
    }));
  };
  ...
  const calculateTotal = () => {
    ...
    const hqExHq = (dailyExpense.hq || dailyExpense.exHq) ? (parseFloat(dailyExpense.hqExHqAmount)) || 0 : 0;
    ...
    <div className="p-4 w-full max-w-md mx-auto bg-gray-50 min-h-screen">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {(dailyExpense.hq || dailyExpense.exHq) && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  {dailyExpense.hq && dailyExpense.exHq ? 'HQ & Ex-HQ' : dailyExpense.hq ? 'HQ' : 'Ex-HQ'} Amount (৳)
                </label>
                <input
                  type="number"
                  value={dailyExpense.hqExHqAmount}
                  onChange={(e) => handleInputChange('hqExHqAmount', e.target.value)}
                  className="w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
        ...
        <div className="bg-white p-4 rounded-lg shadow-md border border-gray-200">
          ...
          <div className="grid grid-cols-1 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Hotel Bill (৳)</label>
              <input
                type="number"
