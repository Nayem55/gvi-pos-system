    ...
    try {
      ...
      if (searchQuery) {
        ...
        response = await axios.get(
          `http://175.29.181.245:2001/search-product?search=${searchQuery}&type=name`
        );
        ...
        response = await axios.get(
          `http://175.29.181.245:2001/products?page=${pageNumber}`
        );
      ...
      const stockRequests = fetchedProducts.map(async (product) => {
        try {
          const stockResponse = await axios.get(
            `http://175.29.181.245:2001/outlet-stock?barcode=${product.barcode}&outlet=${selectedOutlet}`
          );
