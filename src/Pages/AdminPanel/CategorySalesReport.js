...
import { BarChart2, ClipboardCheck, ShoppingCart, DollarSign, X } from "lucide-react";
...
const CategoryWiseSalesReport = () => {
  ...
  const fetchOutletDetails = async (category) => {
    try {
      ...
      const totalQuantity = response.data.reduce((sum, outlet) => sum + outlet.total_quantity, 0);
      const totalTP = response.data.reduce((sum, outlet) => sum + outlet.total_tp, 0);
      const totalMRP = response.data.reduce((sum, outlet) => sum + outlet.total_mrp, 0);
  ...
  const handleExportModalToExcel = () => {
    ...
    const fileName = `OutletSales-${selectedCategory}.xlsx`;
    saveAs(fileData, fileName);
  };
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[80vh] overflow-auto">
            ...
            <div className="p-4">
              {modalLoading ? (
                <p className="text-center text-gray-500">Loading outlet details...</p>
              ) : modalData.length === 0 ? (
