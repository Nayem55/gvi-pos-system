...
const UserDashboard = () => {
  ...
  const fetchDailyReports = async () => {
    try {
      const response = await axios.get(
        `http://175.29.181.245:2001/sales-reports/${user._id}?month=${selectedMonth}`
      );
  ...
  const fetchUserTarget = async () => {
    try {
      ...
      const response = await axios.get("http://175.29.181.245:2001/targets", {
      ...
      await Promise.all(
        stockUpdates.map(
          ({ barcode, newStock, currentStockValueDP, currentStockValueTP }) => {
            return axios.put("http://175.29.181.245:2001/update-outlet-stock", {
              barcode,
              outlet: user.outlet,
              newStock,
              currentStockValueDP,
  ...
  //       `http://175.29.181.245:2001/sales-reports/${reportId}`
  //     );
