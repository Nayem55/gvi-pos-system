...
const BrandTargetPage = () => {
    ...
    const fetchData = async () => {
      try {
        const [usersRes, brandsRes] = await Promise.all([
          ...
          axios.get("http://175.29.181.245:2001/brands"),
        ]);
        ...
        toast.error("Failed to load users and brands");
      }
    };
    fetchData();
  }, []);
  ...
  const saveTargets = async (userID) => {
    ...
    try {
      ...
      await axios.post("http://175.29.181.245:2001/brandTargets", {
        userID,
        year: parseInt(year),
    ...
    try {
      const updates = Object.entries(tempTargets)
        ...
        .map(([userID, targets]) => {
          ...
          return axios.post("http://175.29.181.245:2001/brandTargets", {
            userID,
            year: parseInt(year),
            month: parseInt(month),
            targets: targetsArray,
      ...
      brands.forEach((brand) => {
        if (row[brand] !== undefined && row[brand] !== "") {
          ...
          if (!isNaN(targetValue)) {
            ...
            newTempTargets[user._id][brand] = targetValue.toString();
            processedCount++;
          }
        }
      });
...
export default BrandTargetPage;