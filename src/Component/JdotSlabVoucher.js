...
export default function JdotSlabVoucher({ stock, setStock }) {
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      if (ownerNumber.length > 2) {
        ...
        try {
          const res = await axios.get(
            `http://175.29.181.245:2001/jdot-search-customer?search=${ownerNumber}`
          );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axios.post("http://175.29.181.245:2001/jdot-add-slab-report", payload);

      await axios.post("http://175.29.181.245:2001/jdot-upsert-customer", {
        owner_name: ownerName,
        owner_number: ownerNumber,
        shop_name: shopName,
        shop_address: shopAddress,
